...
genai_client = genai.Client(api_key=KEY_GENAI) if KEY_GENAI else None

def analyze_paper_single_batch(info_json: str, model_name: str = "gemini-flash-latest") -> List[Dict]:
    """Analyze a single batch of research papers using google-genai SDK."""
    if not genai_client:
        raise RuntimeError("KEY_GENAI is not configured or genai_client is not initialized")
    ...
    response = chat.send_message(
        message=[
            types.Part.from_text(text=info_json),
...
def analyze_paper_with_fallback(df_chunk: pd.DataFrame, primary_model: str = "gemini-flash-latest") -> List[Dict]:
    """Analyze paper batch with automatic multi-model fallback and sub-batch splitting on failure."""
    if df_chunk.empty:
        return []

    info_json = df_chunk.to_json(orient='records', force_ascii=False)
    
    # 1. Try Primary Gemini Model (gemini-flash-latest with high thinking)
    try:
        return analyze_paper_single_batch(info_json, model_name=primary_model)
    except Exception as e1:
        print(f"[Warning] Primary model ({primary_model}) failed: {e1}")

    # 2. Try a different Gemini model before splitting
    fallback_model = "gemini-3.6-flash"
    try:
        print(f"[Fallback] Retrying batch ({len(df_chunk)} papers) with fallback model ({fallback_model})...")
        return analyze_paper_single_batch(info_json, model_name=fallback_model)
    except Exception as e2:
        print(f"[Warning] Fallback model ({fallback_model}) failed: {e2}")

    # 3. SiliconFlow OpenAI API Fallback as last resort
    print("[Fallback] Retrying with SiliconFlow OpenAI API...")
    try:
        return analyze_paper_openai(info_json)
    except Exception as e3:
        print(f"[Error] SiliconFlow API fallback also failed: {e3}")
        # Return fallback mock items to keep script from crashing
        return [{"summary": "解析摘要失败", "tags": ["文献", "分析", "错误"], "score": 50.0} for _ in range(len(df_chunk))]

def analyze_paper(info_json: str) -> List[Dict]:
    """Backwards compatible wrapper for analyze_paper."""
    df_temp = pd.read_json(io.StringIO(info_json))
    return analyze_paper_with_fallback(df_temp)
